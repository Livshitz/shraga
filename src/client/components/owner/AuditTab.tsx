...
export function AuditTab({ call, onOpenSession }: { call: OwnerCall; onOpenSession: (sessionId: string) => void }) {
            ...
            {items.map((r) => (
              <tr key={r.hash}>
                ...
                <td className={`${tdCls} whitespace-nowrap text-right`}>
                  ...
                  {r.principal && <button className={`${iconBtn} hover:text-destructive`} title="Block principal" disabled={!!busy} onClick={() => run('bl', () => actions.block(r.principal!))}><Ban className="w-3.5 h-3.5" /></button>}
                </td>
              </tr>
            ))}
