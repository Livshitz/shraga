...
When set, the prompt tells the agent to use the gateway, `share_file` routes media, files over `maxLocalFileMB`,
and anything when the box has no public origin through it — but only when the gateway's links are public; otherwise
small files fall back to the box's own link and the rest are refused (the agent is told to send an attachment), heavy local Bash (ffmpeg encodes, remotion renders, starting mcp-video/mcp-audio)
